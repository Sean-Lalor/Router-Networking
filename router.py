"""Generates a Router instance takes Parsed data and runs it"""
import socket
import select
import sys

# sys.argv[0]
# Need to create a On/Off Switch for the Router
#Only reason we need sys to be imported


def Send(message, ip_instance, port):
    #Creates a Socket instance
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM);
    sock.sendto(message, (ip_instance, port))
    sock.close();   
    print("Msg_Sent");

def Router_Instance(ip_instance, port):
    #Creates a server interface for the router to recieve messages
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM);
    sock.bind((ip_instance, port));
    return sock;

def setup(ID_num, input_ports, out_list, IP_Address):
    
    #Will contain all instances of sockets
    sock_drawer = [0,0,0,0,0]
    
    for port_n in range(0,len(input_ports)):
        sock_drawer[port_n] = Router_Instance(IP_Address, input_ports[port_n])
        
    print(sock_drawer)
    
    Send(message, IP_Address, input_ports[0])
...
def parser(fileText):
    ...
    for port_n in range(0,len(input_list)):
        input_list[port_n] = int(input_list[port_n])
...
IP_host_Address = "127.0.0.1"

router_info = parser(testfile1);
if type(router_info) == type(''):
    print(router_info)
else:
    for i in router_info:
        print(i);
    
setup(router_info[0], router_info[1], router_info[2], IP_host_Address)